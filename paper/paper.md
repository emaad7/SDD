---
title: 'SDD: ‎S‎‎oftware for ‎m‎ultivariate ‎spatial dependence detection via distance ‎correlation‎'
tags:
  - R
  - GUI
  - ‎Spatial Dependence
...
The distance correlation between \( {\bf X} \) and \( {\bf Y} \), denoted as \( R({\bf X}, {\bf Y}) \), introduced by @Szekely2007, measures the dependence between two random vectors. Partial distance correlation, \( R({\bf X}, {\bf Y}; {\bf Z}) \), introduced by @Szekely2014, measures the dependence between two random vectors \( {\bf X} \) and \( {\bf Y} \) after controlling for a third vector \( {\bf Z} \). There are two types of estimators for distance dependence: one based on @Szekely2007 and the other on @Szekely2013a. These are distinguished by an asterisk in the results and are denoted by \( R_n \) and \( R_n^* \), respectively.

The neighborhood dependence indices measure the dependence between observations and their spatial neighbors:

\[
SR_{\bf n}({\bf k}) = R_n ({\bf Z(s,k),Z(s)}), \quad SR^*_n({\bf k}) = R^*_{\bf n}({\bf Z(s,k),Z(s)})
\]

where \( {\bf Z(s,k)} \) represents observations in the \( {\bf k} \)-th neighborhood. To remove regional trends and isolate spatial dependence, we define de-trended indices:

\[
DR_{\bf n}({\bf k}) = R_n({\bf Z(s,k),Z(s)};  g{\bf (s)}), \quad DR^*_{\bf n}({\bf k}) = R^*_n({\bf Z(s,k),Z(s)}; g{\bf (s)})
\]

where \(  g{\bf (s)} \) represents a trend function. The contribution of regional trends to dependence is measured using trend indices:

\[
TR_{\bf n}({\bf k}) = 1 - DR_{\bf n}({\bf k}), \quad TR^*_{\bf n}({\bf k}) = 1 - DR^*_{\bf n}({\bf k}).
\]

To assess dependence influenced by external factors, regional dependence indices incorporate regional variables \( {\bf Y(s)} \):

\[
RR_{\bf n} = R_n({\bf Z(s), Y(s)}), \quad RR^*_{\bf n} = R^*_n({\bf Z(s), Y(s)}).
\]

Aggregated measures over multiple neighborhoods are calculated by averaging the spatial dependence values of specific measures across \( m \) neighborhoods. Specifically, \( NDC(m) \) and \( NDC^*(m) \) represent the averages of the spatial dependence values \( SR_{\bf n}({\bf k}) \) and \( SR^*_{\bf n}({\bf k}) \) for \( m \) neighborhoods, respectively. Similarly, \( NWR(m) \) and \( NWR^*(m) \) are the averages of \( DR_{\bf n}({\bf k}) \) and \( DR^*_{\bf n}({\bf k} \), while \( NRD(m) \) and \( NRD^*(m) \) are the averages of \( TR_{\bf n}({\bf k}) \) and \( TR^*_{\bf n}({\bf k} \). These indices offer a comprehensive approach to evaluating spatial dependence in multivariate data.
...
The SDD software is available on StatisSoft at [https://statisme.com/Home/StatisSoftList](https://statisme.com/Home/StatisSoftList) and [https://github.com/emaad7/SDD](https://github.com/emaad7/SDD). These websites provide access to the software, its manual, and the data used in the article.
